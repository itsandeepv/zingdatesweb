...
import { Heart, MessageCircle, Mail, Phone, Crown, Video, Rocket, Sparkles, X, type LucideIcon } from 'lucide-react'
...
// Map of action keys → hero config (icon, headline, sub)
const ACTION_COPY: Record<string, { Icon: LucideIcon; title: string; sub: string }> = {
  like:    { Icon: Heart,         title: 'Upgrade to Like Profiles', sub: 'Get a plan to send likes and find your match.' },
  chat:    { Icon: MessageCircle, title: 'Upgrade to Chat',          sub: 'Get a plan to open and send messages.' },
  message: { Icon: Mail,          title: 'Upgrade to Send Messages', sub: 'Get a plan to reply and keep the conversation going.' },
  call:    { Icon: Phone,         title: 'Upgrade to Make Calls',    sub: 'Get a plan to make audio & video calls.' },
}
const DEFAULT_COPY = { Icon: Crown, title: 'Upgrade to Premium', sub: 'Unlock everything zingDates has to offer.' }
...
export default function NoPlanModal() {
  ...
  const copy = (action ? ACTION_COPY[action] : undefined) ?? DEFAULT_COPY
      ...
      >
        ...
        <div className="gradient-brand relative px-6 pt-8 pb-14 text-center overflow-hidden">
          ...
          <div className="mb-3 relative z-10 flex justify-center" style={{ filter: 'drop-shadow(0 2px 8px rgba(0,0,0,0.3))' }}>
            <HeroIcon size={46} strokeWidth={2} className="text-white" />
          </div>
        ...
        <div className="px-6 pt-5 pb-2 space-y-3.5">
          {[
            { Icon: MessageCircle, text: 'Send & receive unlimited messages' },
            { Icon: Video,         text: 'Crystal-clear audio & video calls' },
            { Icon: Heart,         text: 'See exactly who liked your profile' },
            { Icon: Rocket,        text: 'Priority matching & weekly profile boost' },
          ].map(({ Icon, text }) => (
